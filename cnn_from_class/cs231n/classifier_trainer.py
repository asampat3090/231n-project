  ...
  def train(self, X, y, X_val, y_val, 
    ...
    for it in xrange(num_iters):
      ...
      if first_it or epoch_end or acc_check:
        ...
        # so we batch it
        y_pred_train = []
        for i in xrange(X_train_subset.shape[0] / batch_size):
          X_train_slice = X_train_subset[i*batch_size:(i+1)*batch_size]
        ...
        y_pred_val = []
        for i in xrange(X_val.shape[0] / batch_size):
          X_val_slice = X_val[i*batch_size:(i+1)*batch_size]
          if predict_fn is not None:
